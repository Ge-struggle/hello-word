...
function result() {
    ...
    let newStory=storyText.replaceAll(':inserta:',xItem).replace(':insertb:',yItem).replace(':insertc:',zItem);


    if(customName.value !== '') {
        let name = customName.value;
        newStory=newStory.replace('李雷',name);
        //不能直接使用newStory.replace('李雷',name);的原因在于string 是不可变的，与对象不同
    }
    ...
    if(document.getElementById("american").checked) {
        ...
        newStory=newStory.replace('130 公斤的胖子',weight+' 磅');
